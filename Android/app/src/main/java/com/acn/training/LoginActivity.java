...
public class LoginActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		loginButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					String defaultUserName = "company";
					String defaultPassword = "company";
					if (userNameEditField.getEditableText().toString().trim().equalsIgnoreCase(defaultUserName) && passwordEditField.getEditableText().toString().trim().equalsIgnoreCase(defaultPassword)) {

						Boolean isRefilled = userPreferences.getBoolean("Refilled", false);
						if (!isRefilled) {
							SharedPreferences.Editor prefsEditor = userPreferences.edit();
							prefsEditor.putBoolean("Refilled", true);
							prefsEditor.putFloat("Balance", INIT_BALANCE);
							prefsEditor.commit();
						}
						finish();
	...
	public String getVersion() {
		...
		try {
			...
			System.err.println("Failed to get 'build.number' property from version_number file.");
			return valueToReturn;
		}
		if (temp != null) {
			if (temp.length() > 0) {
